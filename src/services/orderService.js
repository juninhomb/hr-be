...
class OrderService {
  ...
  //   - Stock só é deduzido quando status === 'pago'.
  //   - Se status === 'aguardando_pagamento', cria o pedido sem mexer
  //     no stock (será deduzido na confirmação).
  // -------------------------------------------------------------
    ...
    try {
      ...
      for (const item of items) {
        ...
        if (deductStock) {
          ...
          variantId = stockUpdate.rows[0].id;
        } else {
          // Sem dedução, mas resolvemos o variant_id na mesma
      ...
      await LogService.register('system', 'order_created', {
        ...
        items: items.length, stock_deducted: deductStock,
      });
  ...
  //   - Se overrideItems vier, sobrescreve os order_items (cobre o
  //     caso do bot n8n não os ter gravado).
  //   - Deduz stock atómicamente.
  //   - status -> 'pago'.
  // -------------------------------------------------------------
  async confirmPayment(orderId, overrideItems = null) {
    ...
    try {
      ...
      if (Array.isArray(overrideItems) && overrideItems.length > 0) {
        ...
        for (const it of overrideItems) {
          ...
          await client.query(
            `INSERT INTO order_items (order_id, variant_id, sku, quantity, unit_price)
             VALUES ($1, $2, $3, $4, $5)`,
            [orderId, variantRes.rows[0]?.id || null, it.sku, it.quantity, it.unit_price ?? null]
          );
        }
      }
      ...
      if (items.length === 0) {
        throw new Error('Pedido sem itens. Envie a lista de itens ao confirmar para deduzir o stock.');
      }
      ...
      for (const it of items) {
        ...
        if (upd.rowCount === 0) {
          throw new Error(`Stock insuficiente para SKU: ${it.sku}`);
        }
      }
      ...
      await client.query('COMMIT');
      await LogService.register('system', 'payment_confirmed', { orderId, items });
  ...
  // -------------------------------------------------------------
  // Cancelar pedido pendente (apenas marca como cancelado)
  // -------------------------------------------------------------
  async cancelOrder(orderId) {
    ...
    if (!rows[0]) throw new Error('Pedido não encontrado ou já processado.');
    await LogService.register('system', 'order_cancelled', { orderId });
    return { success: true, orderId };
  }
  ...
  //   - Se status era 'pago' / 'enviado' / 'entregue': devolve stock
  //     (porque foi deduzido na confirmação ou na criação).
  //   - Se 'aguardando_pagamento' ou 'cancelado': não há stock a repor.
  //   - CASCADE em order_items remove as linhas automaticamente.
  // -------------------------------------------------------------
  async deleteOrder(orderId) {
    ...
    try {
      ...
      const order = orderRes.rows[0];
      const stockWasDeducted = ['pago', 'enviado', 'entregue'].includes(order.status);
