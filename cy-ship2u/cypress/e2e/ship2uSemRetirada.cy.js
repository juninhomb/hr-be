  ...
  it('login, new shipment: Way2U + pickup sender, country, volumes, weight', () => {
    ...
    cy.fixture('login').then(({ username, password }) => {
      loadRecipient().then((recipient) => {
        ...
        // «Sem recolha» — marca após volume/peso. Este script é o fluxo
        // de levantamento na loja (cliente entrega o pacote ao estafeta sem
        // recolha agendada). O input é um checkbox `name="without_pickup"`.
        cy.get('#modal-remote-xl input[name="without_pickup"]')
          .check({ force: true })
        cy.get('#modal-remote-xl input[name="without_pickup"]')
          .should('be.checked')
